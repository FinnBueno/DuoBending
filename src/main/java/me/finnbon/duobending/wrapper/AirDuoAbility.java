 ...
 * @author Finn Bon
 */
public abstract class AirDuoAbility extends AirAbility implements DuoAbilityVerifier {

	public List<BendingPlayer> players;

	public AirDuoAbility(Player...players) {
		this(Arrays.stream(players).map(BendingPlayer::getBendingPlayer).toArray(BendingPlayer[]::new));
	}

	public AirDuoAbility(BendingPlayer ...players) {
