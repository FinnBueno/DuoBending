 ...
 * @author Finn Bon
 */
public abstract class CombustionDuoAbility extends CombustionAbility implements DuoAbilityVerifier {

	public List<BendingPlayer> players;

	public CombustionDuoAbility(Player...players) {
		this(Arrays.stream(players).map(BendingPlayer::getBendingPlayer).toArray(BendingPlayer[]::new));
	}

	public CombustionDuoAbility(BendingPlayer ...players) {
